...
const register = async (email, password, apiKey, urlApiKey, setToken, setIsLoggedIn, navigate, setErrorMessage, setLoading) => {
    ...
    const options = {
      ...
      body: JSON.stringify({
         email, password, api_key: apiKey, url_api_key: urlApiKey
      }),
    };

    setLoading(true);
    const response = await fetch(url, options).catch(() => setErrorMessage("Wystąpił błąd"));
    setLoading(false);
...
const logIn = async (email, password, setToken, setIsLoggedIn, navigate, setErrorMessage, setLoading) => {
...
const AuthScreen = ({ setToken, setIsLoggedIn }) => {
    ...
    const tryRegister = () => register(email, password, apiKey, urlApiKey, setToken, setIsLoggedIn,
        navigation.navigate, setErrorMessage, setLoading);
    const tryLogin = () => logIn(email, password, setToken, setIsLoggedIn,
        navigation.navigate, setErrorMessage, setLoading);

    return (
        <ImageBackground source={require('./memphis-mini.png')} style={styles.backgroundImage}>
